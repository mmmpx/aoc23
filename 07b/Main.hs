...
jcounts :: Hand -> [(Card, Int)]
jcounts h
  = let (js, cs) = partition ((== card 'J') . fst) $ sort $ counts h
        j = sum $ map snd js
    in case cs of
      [] -> [(card 'J', j)]
      _  -> let mx = maximum $ map snd cs
                mxcs = filter ((== mx) . snd) cs
                mxc = maximum $ map fst mxcs
                rst = filter ((/= mxc) . fst) cs
            in rst ++ [(mxc, mx+j)]
...
rank :: Hand -> HandType
rank = fromJust . flip lookup ranks . (sort . map snd . jcounts)
...
solve :: [(Hand, Bid)] -> Int
solve = sum . zipWith (*) [1..] . map snd . sortBy (comparing fst)
